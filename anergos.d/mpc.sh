...
function check_val() { 
	if [ $1 ] && [ $1 -eq 0 ]; then
		echo $(tput setaf 1)"${FUNCNAME[1]} skipped"$(tput sgr0) && return 69
	fi
}

function nobeep() {
	check_val $1; [ $? -eq 69 ] && return
...
function agetty_set() {
	systemctl enable gdm >/dev/null 2>&1 && echo "Enabled gdm" && return
	check_val $1; [ $? -eq 69 ] && return
	if [ "$1" = "auto" ]; then
...
[ $hostname = "killua" ] && { resolv_conf 1; enable_numlk_tty 1; temps 1; data 1; }
[ $hostname = "killua" ] && powerb_is_suspend 0
