...
function virtualbox() {
	...
	if [[ $(lspci | grep VirtualBox) ]]; then
		if [ -f /usr/bin/pacman ]; then
			...
			echo $(tput setaf 2)"${FUNCNAME[0]}-guest done"$(tput sgr0)
		else
			echo $(tput setaf 1)"${FUNCNAME[0]} skipped (pacman is not installed)"$(tput sgr0)
		fi
		...
		echo $(tput setaf 2)"${FUNCNAME[0]}-host done"$(tput sgr0)
	fi
}
